...
import { projectsDataProvider } from "./ProjectsApp"
...
const ProjectsShowContent = () => {
  ...
  const handleBack = useCallback(
    (event: MouseEvent) => {
      navigateWithModifier(event, "/projects", router)
    },
    [router],
  )
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        Modal.confirm({
          ...
          content: `#${invoiceNumber} — the invoice will be archived and related GL entries will be voided`,
          okType: 'danger',
          async onOk() {
            try {
              const endpoint = `/api/projects/by-id/${encodeURIComponent(project.id)}/invoices`
            ...
            <div className="descriptor-actions">
                <Button danger onClick={async () => {
                  ...
                  Modal.confirm({
                    ...
                    content: `#${project.projectNumber} — the project and all invoices will be archived; GL entries will be voided`,
                    okType: 'danger',
                    async onOk() {
                      try {
