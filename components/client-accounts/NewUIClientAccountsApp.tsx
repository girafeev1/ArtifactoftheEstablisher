    ...
    switch (field) {
      ...
      case "hasOverduePayment": {
        ...
        if (Array.isArray(value)) {
          ...
          return result.filter((row) => normalized.includes(String(row.hasOverduePayment)))
        }
        if (typeof value === "string") {
          const trimmed = value.trim()
          if (trimmed === "true" || trimmed === "false") {
            return result.filter((row) => String(row.hasOverduePayment) === trimmed)
          }
        }
    ...
    >
      ...
      >
          ...
          filterDropdown={(props) => (
            <FilterDropdown {...props}>
              <Select
                ...
                allowClear
                options={[
                  { value: "false", label: "No Payment Due" },
                  { value: "true", label: "Payment Due" },
                ]}
              />
            </FilterDropdown>
          )}
        ...
        <Table.Column<ClientAccountRow>
          ...
          render={(_, record) => (
            <Space align="start">
              <CustomAvatar seed={record.avatarSeed} value={record.baseName ?? record.displayName} />
