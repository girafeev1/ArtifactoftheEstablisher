...
const PaymentStatusTag = ({ overdue }: { overdue: boolean }) => (
  <Tag color={overdue ? "red" : "green"} style={{ borderRadius: 999, textTransform: "capitalize" }}>
    <Space size={6}>
      ...
      {overdue ? "Payment Due" : "Payment Cleared"}
    </Space>
  </Tag>
)
...
const CardSection = ({ title, value, showLabel = true, onClick }: CardSectionProps) => (
  <div>
    ...
    <Text
      strong
      style={
        onClick ? { color: "#1d4ed8", cursor: "pointer", display: "inline-block" } : undefined
      }
      onClick={(event) => {
        if (!onClick) {
          return
...
const ClientDetailsDrawer = ({
  ...
  const hydrateValues = (entry: ClientAccountRow): ClientDetailsFormValues => ({
    ...
    addressLine4: entry.source.addressLine4 ?? entry.addressLine4 ?? null,
  ...
  const detailFields: DetailFieldConfig[] = useMemo(() => {
    ...
    return [
      ...
      },
      {
        key: "phone",
        label: "Phone",
        display: <Text strong>{formatDisplayValue(record.phone)}</Text>,
        editable: true,
        fields: [
          {
            name: "phone",
            node: <Input placeholder="Enter phone" />,
          },
        ],
      },
      {
        ...
        editable: true,
        fields: [
          { name: "addressLine1", node: <Input placeholder="Address line 1" /> },
          { name: "addressLine2", node: <Input placeholder="Address line 2" /> },
          { name: "addressLine3", node: <Input placeholder="Address line 3" /> },
          { name: "addressLine4", node: <Input placeholder="Address line 4" /> },
          {
            name: "region",
            node: (
              <Select
