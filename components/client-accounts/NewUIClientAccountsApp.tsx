...
const AlphabetScrollbar = ({ active, available, onSelect }: AlphabetScrollbarProps) => {
  ...
  const pickToken = (clientY: number) => {
    ...
    const index = Math.min(
      alphabetTokens.length - 1,
      Math.max(0, Math.floor(ratio * alphabetTokens.length)),
    )
    const token = alphabetTokens[index]
    ...
    >
      ...
      >
        ...
        <Table.Column<ClientAccountRow>
          ...
          render={(_, record) => (
            <Space align="start">
              <CustomAvatar seed={record.avatarSeed} value={record.displayName} />
