...
interface ProjectData {
  ...
  projectDate: string; // Expected to be stored as an ISO date string ("yyyy-MM-dd") or convertible to one.
  agent: string;
...
export default function EditProjectDialog({
  ...
  // Filter bank accounts by the file's company name, if provided.
  ...
  // Local state for the bank dropdowns.
  const [selectedBank, setSelectedBank] = useState('');
  const [selectedAccountType, setSelectedAccountType] = useState('');

  // When the dialog opens or when the project's bankAccountIdentifier changes,
  // set the local bank/account dropdown values.
  useEffect(() => {
    console.log('[EditProjectDialog] safeBankAccounts =>', safeBankAccounts);
    if (project.bankAccountIdentifier && project.bankAccountIdentifier.trim() && safeBankAccounts.length > 0) {
    ...
    setProject(prev =>
      prev
        ? {
            ...
            // If unchecking, you might clear paidOnDate; otherwise keep current.
            paidOnDate: checked ? prev.paidOnDate : '',
            // Do not force-reset bankAccountIdentifier here so that user edits aren’t lost.
          }
        : prev
    );
    if (!checked) {
  ...
  // When the user changes the bank selection, update the local state.
  // (Do not update the project state until an account type is chosen.)
  function handleChangeBank(e: React.ChangeEvent<{ value: unknown }>) {
    const newBank = e.target.value as string;
    setSelectedBank(newBank);
    setSelectedAccountType('');
    // Do not reset project.bankAccountIdentifier here.
  }

  // When the user selects an account type, update local state and update the project.
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <TextField
          label="Project Date"
          type="date"
          // Convert the stored projectDate to ISO format for the date input.
          value={project.projectDate ? new Date(project.projectDate).toISOString().split('T')[0] : ''}
          onChange={(e) =>
