...
const CoachingSessionsApp = dynamic(
  () => import("../components/coaching/CoachingSessionsApp"),
  { ssr: false }
)
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  console.info("[coaching] getServerSideProps invoked", requestContext)

  try {
    const session = await getSession(ctx)

    if (!session?.user) {
      console.warn("[coaching] No authenticated session, redirecting", requestContext)
      return {
        redirect: {
    ...
    console.info("[coaching] Authenticated session detected", {
      ...requestContext,
      user: identity,
    })
    ...
    console.error("[coaching] getServerSideProps failed", {
      ...requestContext,
      error:
