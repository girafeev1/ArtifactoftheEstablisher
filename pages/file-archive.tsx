...
const FileArchiveApp = dynamic(
  () => import("../components/file-archive/FileArchiveApp"),
  { ssr: false },
)
...
export default function FileArchivePage() {
  return (
    <>
      <Head>
        <title>File Archive · AOTE PMS</title>
      </Head>
      <FileArchiveApp />
    </>
  )
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  console.info("[file-archive] getServerSideProps invoked", requestContext)

  try {
    const session = await getSession(ctx)

    if (!session?.user) {
      console.warn("[file-archive] No authenticated session, redirecting", requestContext)
      return {
        redirect: {
    ...
    console.info("[file-archive] Authenticated session detected", {
      ...requestContext,
      user: identity,
    })
    ...
    console.error("[file-archive] getServerSideProps failed", {
      ...requestContext,
      error:
