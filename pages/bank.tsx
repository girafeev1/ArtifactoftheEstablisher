...
const FinanceApp = dynamic(
  () => import("../components/finance/FinanceApp"),
  { ssr: false },
)
...
export default function BankPage() {
  return (
    <>
      <Head>
        <title>Bank Access · AOTE PMS</title>
      </Head>
      <FinanceApp />
    </>
  )
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  console.info("[bank] getServerSideProps invoked", requestContext)

  try {
    const session = await getSession(ctx)

    if (!session?.user) {
      console.warn("[bank] No authenticated session, redirecting", requestContext)
      return {
        redirect: {
    ...
    console.info("[bank] Authenticated session detected", {
      ...requestContext,
      user: identity,
    })
    ...
    console.error("[bank] getServerSideProps failed", {
      ...requestContext,
      error:
