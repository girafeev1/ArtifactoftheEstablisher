...
import { Button, Space, Typography, Empty, Spin, Switch, Radio } from "antd";
...
type RenderMode = 'classic' | 'dynamic';

export default function InvoicePreviewEnhancedPage() {
  ...
  // New state for dynamic rendering
  ...
  // Fetch dynamic invoice layout when mode changes to dynamic
  useEffect(() => {
    if (renderMode !== 'dynamic' || !year || !projectId || !invoiceNumber) return;
  ...
  }, [renderMode, year, projectId, invoiceNumber]);
  ...
  }, [isReadyForPrint, invoice, project, subsidiary, bankInfo, renderMode]);

  const isDynamicReady = renderMode === 'dynamic' && !dynamicLoading && !!composedLayout;
  const canRenderDynamic = renderMode === 'dynamic' && isReadyForPrint && isDynamicReady;
  const canRenderClassic = renderMode === 'classic' && isReadyForPrint;
        ...
        <div className="preview-inner">
          ...
          <div style={{ marginBottom: 16, padding: '12px', background: '#f5f5f5', borderRadius: '6px' }}>
            <Space direction="vertical" size={8} style={{ width: '100%' }}>
              <Text strong>Rendering Engine:</Text>
              ...
              {renderMode === 'dynamic' && dynamicLoading && (
                <Text type="secondary">Loading dynamic layout...</Text>
              )}
              {renderMode === 'dynamic' && !dynamicLoading && composedLayout && (
                <Text type="success">
          ...
          <div className="viewer-wrap">
            {loading ? (
              ...
              >
                {renderMode === 'classic' && canRenderClassic && (
                  <GeneratedInvoice
                    invoice={invoice}
                ...
                {renderMode === 'dynamic' && canRenderDynamic && composedLayout && (
                  <DynamicInvoice
                    invoice={invoice}
