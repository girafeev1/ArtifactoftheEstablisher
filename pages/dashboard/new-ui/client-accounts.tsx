    ...
    <List
      ...
      pagination={{
        ...
        onChange: (page, pageSize) => {
          setCurrent(page)
          setPageSize(pageSize ?? pagination?.pageSize ?? 12)
        },
        showTotal: (total) => <PaginationSummary total={total} />,
      }}
    >
      ...
      >
        {(loading ? Array.from({ length: 12 }).map((_, index) => ({ ...placeholderCard, id: `placeholder-${index}` })) : data).map(
          (record) => (
            <ClientCard key={record.id} record={record} loading={loading} />
          ),
        )}
      </div>
    </List>
  ...
  return (
    <div style={{ padding: 32, minHeight: "100%", background: "#f5f7fb" }}>
      ...
      {view === "table" ? (
        ...
        <ClientCards tableProps={tableProps} setCurrent={setCurrent} setPageSize={setPageSize} />
      )}
    </div>
  )
}
