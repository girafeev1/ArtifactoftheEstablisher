...
export default function InlineEdit({
  ...
  const save = async (v: any) => {
    ...
    try {
      ...
      const ref = doc(db, col, docId, collectionName, docName)
      await setDoc(ref, {
        [fieldKey]: v,
        timestamp: today,
      })
      ...
      if (verify.exists()) {
        const stored = (verify.data() as any)[fieldKey]
        setDraft(stored)
      } else {
        alert('Save failed to verify. Please retry.')
      }
    } catch (e) {
  ...
  const display = () => {
    if (type === 'date' && draft) {
      const d = draft instanceof Timestamp ? draft.toDate() : new Date(draft)
